...
class ApiClient {
  ...
  Future<HotspotResponse> getHotspots(double lat, double lng, double radius) async {
    final url = Uri.parse('$baseUrl/api/places?latitude=$lat&longitude=$lng&radius=$radius');
  ...
  Future<List<dynamic>> fetchPlaceSuggestions(String input, String sessionToken) async {
    final url = Uri.parse('$googlePlacesBaseUrl/autocomplete/json?input=$input&key=$apiKey&sessiontoken=$sessionToken');
  ...
  Future<LatLng> selectPlace(String placeId, String sessionToken) async {
    final url = Uri.parse('$googlePlacesBaseUrl/details/json?place_id=$placeId&fields=geometry&key=$apiKey&sessiontoken=$sessionToken');
    final response = await http.get(url);
  ...
  Future<NearbyChargersResponse> fetchNearbyChargers(Map<String, dynamic> requestBody) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to fetch nearby chargers: ${response.statusCode}');
    }
  }
}